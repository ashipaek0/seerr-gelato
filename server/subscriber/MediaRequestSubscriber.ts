...
// invalidate each other's tokens. The pending promise ensures concurrent
// callers wait for the in-flight authentication instead of starting their own.
...
export class MediaRequestSubscriber implements EntitySubscriberInterface<MediaRequest> {
  ...
  public async sendToGelato(entity: MediaRequest): Promise<void> {
    ...
    // Resolve session token early — before any await — so concurrent
    // callers all wait on the same in-flight authentication promise.
    ...
    if (jellyfinUsername && jellyfinPassword) {
      if (
        ...
        // Token still valid — no async work needed
      } else if (!pendingAuth) {
        // Synchronously check-and-set before any await
      ...
      if (pendingAuth) {
        sessionTokenPromise = pendingAuth;
      }
    }
    ...
    try {
      ...
      // Resolve session token (wait for in-flight auth if needed)
      let sessionToken: string | undefined;
      if (sessionTokenPromise) {
        try {
          sessionToken = await sessionTokenPromise;
        } catch (e) {
          logger.warn('Session token auth failed, using API key fallback', {
            label: 'Gelato',
            error: e.message,
          });
        }
